...
#define PROJECTNAME "ECS0026 _stdlib"
...
// globals
	...
	// hc595
		volatile uint8_t _hc595_buffer[2];
	// seg7led
		volatile char _seg7led_buffer[4];
		volatile char _seg7led_pointbuffer[4];
		volatile uint8_t _seg7led_digitLoop = 0;
	// adc
		volatile uint16_t _adc_mean;
#define ADC_N 3
...
/*		seg7led_setPoint(0, 0);
		...
		while(1){
			...
			_adc_mean += (ADC1->DR - _adc_mean) >> ADC_N;
			seg7led_setValue(_adc_mean);
...
seg7led_init(void){
	...
	TIM3_NVIC_Init();
	memset((void*)_seg7led_pointbuffer, 0, 4);
